...
def scp_iteration(f, s0, s_goal, s_prev, u_prev, N, P, Q, R, u_max, ρ):
    ...
    n = Q.shape[0]
    m = R.shape[0]
    s_cvx = cvx.Variable((N + 1, n)) # N is the total time steps; n is dimension of s_k
    ...
    # cost_stage_s = cvx.sum([cvx.quad_form(s_cvx[i,:]-s_goal, Q) for i in range(N)])
    
    # cost_stage_s = [cvx.quad_form(s_cvx[i,:]-s_goal, Q) for i in range(N)]

    
    # cost_stage_u = cvx.sum([cvx.quad_form(u_cvx[i,:], R) for i in range(N)])

    # maybe replace cvx.sum 
    ...
    constraints += [ cvx.max(cvx.abs(u_cvx)) <= u_max] # Constraint 2: Control signals within a set of bounds [-u_max, u_max]
        # maybe pick out the max of the matrix
    # constraints += [ cvx.abs(s_cvx[i,] - s_prev[i]) <= ρ for i in range(N+1)] # Constraint 3a: Trusted Region for s

    # constraints += [ cvx.max(cvx.abs(s_cvx - s_prev)) <= ρ] # Constraint 3a: Trusted Region for s

    # breakpoint()
    for i in range(N+1):
        for j in range(n):
            constraints += [cvx.abs(s_cvx[i,j] - s_prev[i,j])<= ρ]

    # constraints += [ cvx.max(cvx.abs(u_cvx - u_prev)) <= ρ] # Constraint 3a: Trusted Region for u
        # Perhaps try a for loop for the trust region constraints
    
    
        # for j in range(N):
    constraints += [cvx.abs(u_cvx[i] - u_prev[i])<= ρ for i in range(N)]
    
    # # constraints += [ cvx.abs(s_cvx[:,-2:] - s_prev[np.newaxis,-2:]) <= ρ] # Constraint 3b: Trusted Region for s
    # constraints += [ cvx.abs(u_cvx - u_prev[np.newaxis]) <= ρ] # Constraint 4: Trusted Region for u
    # constraints += [ u_cvx - u_prev <= ρ] # Constraint 4: Trusted Region for u
    # constraints += [ -u_cvx + u_prev >= ρ] # Constraint 4: Trusted Region for u
    # breakpoint()
    constraints += [ s_cvx[i+1,:] == A[i,:,:] @ s_cvx[i,:] + B[i,:,:] @ u_cvx[i,:] + c[i,:] for i in range(N)] # Constraint 5: Linearized Dynamics
    # constraints += [slack >= 0]
